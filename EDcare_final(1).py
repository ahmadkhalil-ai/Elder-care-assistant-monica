...
def camera_monitor_loop():
    ...
    while True:
        ...
        cv2.imshow('Monica Elder Care Monitor', display_frame)
        _camera_window_open = True
        
        if (gesture_detected != "none" and 
            current_time - _last_gesture_time > _gesture_cooldown and
            current_time - _last_interaction_time > 2):
            
            if confirm_gesture_with_temporal_analysis(gesture_detected):
                _last_gesture_time = current_time
                handle_gesture(gesture_detected)
...
def listen_once(timeout=15, phrase_time_limit=15):
    global _error_cnt, _spoken_for_this_turn, _last_interaction_time, _is_speaking
...
def handle_emergency(emergency_type="Voice Emergency Trigger"):
    ...
    global _emergency_mode, _awaiting_response
